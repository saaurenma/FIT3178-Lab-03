...
class CurrentPartyTableViewController: UITableViewController, AddSuperHeroDelegate {
        ...
        tableView.performBatchUpdates({
            if marvelHeroNames == [] && DCHeroNames == []{
                ...
                if newHero.universe?.rawValue == SECTION_MARVEL {
                    ...
                    tableView.insertRows(at: [IndexPath(row: (currentParty[SECTION_MARVEL].count) - 1, section:
                SECTION_MARVEL)],
                        with: .automatic)
                }
                    ...
                    tableView.insertRows(at: [IndexPath(row: (currentParty[SECTION_DC].count) - 1, section:
                SECTION_DC)],
                        with: .automatic)
                }
    ...
    override func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        case SECTION_DC:
            return currentParty[SECTION_DC].count
            
        case SECTION_MARVEL:
            return currentParty[SECTION_MARVEL].count
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.section == SECTION_MARVEL{
              print("marvel")
            ...
            let hero = currentParty[SECTION_MARVEL][indexPath.row]
        ...
        else if indexPath.section == SECTION_DC {
            print("dc")
            let heroCell = tableView.dequeueReusableCell(withIdentifier: CELL_HERO, for: indexPath)
          ...
          let hero = currentParty[SECTION_DC][indexPath.row]
            ...
            if currentParty[SECTION_MARVEL].isEmpty && currentParty[SECTION_DC].isEmpty{
    ...
    override func tableView(_ tableView: UITableView, canEditRowAt indexPath: IndexPath) -> Bool {
        ...
        if indexPath.section == SECTION_MARVEL || indexPath.section == SECTION_DC {
            return true
        }
    ...
    override func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        ...
        if editingStyle == .delete && indexPath.section == SECTION_DC {
            tableView.performBatchUpdates({
                self.currentParty[SECTION_DC].remove(at: indexPath.row)
                self.tableView.deleteRows(at: [indexPath], with: .fade)
                self.tableView.reloadSections([SECTION_INFO], with: .automatic)
            }, completion:nil)
        }
        
        else if editingStyle == .delete && indexPath.section == SECTION_MARVEL {
            tableView.performBatchUpdates({
                self.currentParty[SECTION_MARVEL].remove(at: indexPath.row)
                self.tableView.deleteRows(at: [indexPath], with: .fade)
                self.tableView.reloadSections([SECTION_INFO], with: .automatic)
            }, completion:nil)
        }
